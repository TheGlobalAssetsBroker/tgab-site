...
function OfferCard({ name, accent, description, items, to }) {
  return <article className={`offer-card ${accent}`}><span className="offer-label">Account tier</span><h3>{name}</h3><p>{description}</p><ul>{items.map((item) => <li key={item}>{item}</li>)}</ul><Link className="button button-orange" to={to}>{name === "Both tiers" ? "Compare Tiers" : `Choose ${name}`} <img className="upper-right-arrow" src="/images/upper-right-arrow.png" alt="" aria-hidden="true" /></Link></article>;
}
